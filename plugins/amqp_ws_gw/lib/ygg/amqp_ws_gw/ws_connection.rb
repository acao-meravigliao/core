...
#    attr_accessor :partial
  ...
  def initialize(env:, socket:, headers:, session:, routes_config:, debug:, **args)
    ...
    raise "Gateway missing" if !AM::Registry[:amqp_gateway]

    actor_initialize(monitored_by: AM::Registry[:amqp_gateway], **args)
  end
  ...
  def actor_boot
    ...
    actor = AM::Registry[:amqp_gateway].send(:actor)

    @register_fut = AM::Registry[:amqp_gateway].ask(Gateway::MsgRegisterConnection.new(
...
#
#        AM::Registry[:amqp_gateway].tell(Gateway::MsgUnsubscribe.new(exchange_name: ex_name))
#      end
#
#      @socket = nil
#
...
#    @driver.start
  ...
  def message_received(msg)
    ...
    transmit({
      ...
      reply_to: msg[:request_id],
      content_type: 'application/problem+json',
      payload: e.as_json_problem(safe_receiver: Rails.application.config.amqp_ws_gw.safe_receiver), # FIXME add base_uri:
...
log.exception e
  ...
  def subscribe_exchange(exchange)
    ...
    if !@exchanges[exchange]
      ...
      AM::Registry[:amqp_gateway].tell(Gateway::MsgSubscribe.new(exchange_name: exchange))
    end
  ...
  def unsubscribe_exchange(exchange)
    ...
    if @exchanges[exchange] == 0
      ...
      AM::Registry[:amqp_gateway].tell(Gateway::MsgUnsubscribe.new(exchange_name: exchange))
    end
  end
