...
class Connection
  ...
  def initialize(server:, socket:, remote_endpoint_real:, instance_id:, welcome_extra_params:, headers:, calls_to: nil, events_to: nil, **args)
    @server = server
    @ruby_sock = socket
...
##    @auth_manager = auth_manager
  ...
  def actor_boot
    ...
    #@controller_factory = AM::DataStore::MVC::ControllerFactory.new(ds: @ds, prefix: @controller_prefix, aaa_context: @session)
...
##    end
    ...
    transmit_msg({
      type: 'welcome',
      instance_id: @instance_id,
      session: @session && @session.representation,
      online: @online,
      pars: @pars.as_hash,
...
##    res = @ds.ask(AM::DataStore::Store::MsgSelect.new(descr: msg[:body], bind: params[:bind], selection_id: params[:selection_id])).value

    transmit_req_ok(reply_to: msg, body: {
      objects: res.objects.map { |x| x.as_jsonapi },
      relations: res.relations.map { |x| x.as_json },
